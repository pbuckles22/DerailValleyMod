...
- [x] **Epic 1 — Diagnostic HUD** *(HIGH · Journey Stage 1)* — Read-only situational awareness; HUD left → right; **no game-state writes**. **Status: complete — 2026-07-23**
  ...
  *Why now:* Apprentice “don’t blow up” — ammeter / TM / fluids before deep yard tools.
  ...
  - [ ] **1.16 Recommended Limit + soft brake lead** — `Limit` is a **look-ahead recommendation**: adopts the next slower posted board once inside the soft-brake lead (not when tires pass). HUD labels **`(Posted)`** / **`(Recommended)`** / `(Geometry)`; **5 s loosen-hold** and no Posted↔Recommended bounce on the same km/h.
    ...
    - **Adopt hysteresis (0.5.54–0.5.56)** — once a slower board is adopted, keep it until outside a wider release lead (`ReleaseLeadFactor` 1.55 vs adopt 1.15) so grade/speed wobble cannot 30↔60 the Recommended chip. **0.5.55:** sticky release floors speed; scan-drop retain; standstill freeze. **0.5.56:** sticky release keeps adopt-time grade (easing descent cannot un-adopt); `StickyAdoptedLimit` never clobbers tighter sticky with a looser Resolve.
    - **Drive debug (0.5.57)** — `T2 limit` logs throttle / train+ind brake / pipe / grade / mass + Brake `adv=` level; logs also fire when brake crosses idle/light/medium/hard buckets so hard-brake moments are visible for lead tuning.
    - **Honest braking + calmer color (0.5.58)** — 0.5.57 proved a 37.8 t DE2 at full train+ind brake slowed only 69→49 over ~470 m on ≈−2%; hard budgets retuned `0.55/0.30 → 0.25/0.15` m/s² so red arrives earlier. Limit caution is asymmetric: yellow `Limit−10` through `Limit+5`, red above `+5`. Once Limit adopts N, Brake no longer echoes `Brake N`; it advances only to a genuinely tighter next board.
    - **Severe far-drop lead (0.5.59)** — nearby 60 can no longer hide a farther 30. Route scan expands dynamically (capped 6 km) from current speed, consist mass, grade, and locomotive profile; Brake selects the tightest board whose estimated slowdown window is open at **required time ×1.5**. DE2 uses the 0.5.57 calibration; uncalibrated types use a conservative 0.8 deceleration factor and are logged in `T2 limit`.
    - **Guaranteed lead + no blinking (0.5.60)** — 0.5.59 smoke **FAIL** on all three counts, root-caused in `Player.log`. (a) The 0.5.58 “don’t echo the adopted Limit” rule silenced the chip at the exact frame Limit adopted the restriction (line 1815: `Limit 30 (Recommended)`, board 1 780 m out, `adv=None`); the Brake target now qualifies against **speed**, never against the Limit chip, and only needs `MinTargetDeltaKmh` 3 km/h of speed to shed. (b) The window is now the worse of a comfortable slowdown and `GuaranteedStopDistanceMeters` (heavy application, net deceleration floored at `MinNetDecelMps2` so a runaway grade still quotes finite room), **+50%** — never narrower than 0.5.59. (c) `adv=Advisory 50` ↔ `adv=None` chatter on consecutive frames came from raw grade jitter (−2.6 → −0.3 → +0.1) moving the window edge under a board sitting on it: planning grade is now safety-biased (`AdverseGradeHold` — descents apply at once, friendlier readings earned at 0.4 %/s), an on-screen target keeps a 1.35× wider window, and a target rides through a lost scan frame for up to 2 s by closing the distance itself. `T2 limit` gains `scan=/path=/reach=/ahead=/min=/planGrade=` so a late chip can be told apart from a board the route walk never saw.
    - **Tuning store** — append-only JSONL under [`doc/tuning/`](doc/tuning/) (`knobs.jsonl` + `sessions.jsonl`) for long-haul dial/geo/stress/lookahead trends; raw frame dumps optional/local. Prefer JSONL over SQLite until query scale demands it.
    - **Aggressiveness dial (0.5.62–0.5.68)** — Dial **0→1** (sticky-safe→late). Smoke path: 0.1/0.2 sticky → 0.5 Brake/Limit split → **0.40** still Rec30-sticky on long hauls (`2026-07-31_067h`: 483/789 Lim30 Rec, ~10 real takes). **0.5.66:** `BrakeLimitAlign`. **0.5.68 (corpus retune, T2 held):** dial **0.80**, planning margin **10%**, Brake window cap **4.5 km**. Tune store `doc/tuning/`.
    - **Session Drive chip (0.5.67)** — Train-bar `Drive` odometer for smoke; **removed from HUD @ 0.5.79** (odometer still internal for Align rem/trip).
    - **Paused for Align “Google Maps”** — Player waived further 1.16 Tier 2 until **3.5** ships shortest/fastest route + complete switch throws (see TECH_DEBT / handoff).
    - **Next after 3.5 → 1.17** — Posted/speed index on the path graph (stop live board-scan stutters). Do **not** start until **3.5** is smoked + committed.

  - [ ] **1.17 World speed cache (Limit + Align share)** — **Next after 3.5.** Bake posted boards (+ geometry already on graph) into a **track-keyed index** at map build / slow refresh; Limit/Brake **query** route-ahead instead of `FindObjectsOfType<SignDebug>` + per-sign scan every few seconds. Same cache can enrich Dijkstra planning speed when boards known. Posted remains authority; streaming signs still need occasional index refresh. Goal: kill cruise stutter / freezes from board discovery. See TECH_DEBT.
    > As a driver, I want Limit and route ETA to use one world speed map loaded with the track graph so the game does not hitch while scanning every speed board on the fly.
    - **Geometry-ahead scan — the actual 70→30 root cause (0.5.61)** — 0.5.60 smoke still missed the 70→30 warning. New `scan=/path=/reach=` diagnostics caught it live: `path=7seg reach=6971m` (route walk *did* reach the board 1 774 m out) but `ahead=2 min=70@465m` right up to the frame Limit adopted 30 — the route graph was known, but the sign's `SignDebug` GameObject had not streamed in yet (`RefreshSignDebugCacheIfNeeded` only sees currently-instantiated props; Derail Valley streams sign decor near the train, unlike the `RailTrack` graph/curve data, which the route walk already proves loads 6–7 km out). No lookahead-window tuning could ever fix a board the game had not spawned. Fix: `TrackPathAhead.Segment` now carries its `RailTrack`; `SpeedLimitGeometryZones.TryGoverningZone` (reused from the existing SignPlacer-ladder geometry fallback) finds the tightest curve-radius zone **and where it starts** for every walked segment, so a synthetic `AheadBoard` is added from curve data alone — independent of sign streaming — feeding both Recommended adopt and Brake target selection. `T2 limit` gains `geo=N` (synthetic boards added this scan) alongside `ahead=`. **Issue #2 (same session):** Brake could still fire red while the Limit chip itself was only yellow (e.g. `Brake 40` at 44 km/h against a 40 board). `MinTargetDeltaKmh` now equals `SpeedLimitDisplay.NearAboveKmh` (5, was 3) — Brake and Limit share one yellow-tolerance definition.
    - **Grade-aware braking (0.5.51 / 0.5.58)** — two budgets: soft (light `0.18` / heavy `0.08` m/s², 12 s reaction) drives the yellow window and adopt; hard (light `0.25` / heavy `0.15`, 3 s) drives red. Downhill gravity (`9.81 × −grade/100`) is subtracted from both, so a descent warns far earlier. Gravity beating the hard budget = explicit **`RUNAWAY — Brake N NOW`** chip rather than a braking distance the train cannot achieve. Adopt lead factor cut `3.5 → 1.15` (the chip warns early; the number changes when you must act).
  ...
  **Build order:** **Epic 4** / **Epic 2** complete. Resume **1.16** Tier 2 only after **3.5** + **1.17** (stutter) unless player directs otherwise. *(Do not reopen **1.8** HUD thermal prediction.)*
...
- [ ] **Epic 3 — Yard Master / Dispatcher** *(Journey Stages 2–3)* — CTC-style **Align Route**: pick **city → track**, pathfind (through-lane bias), show reverse cue, throw switches. Never delete cars. Scope: **yard and inter-city**. DV has **no Dispatcher license** — gate Align on **Dispatcher** (Dispatcher1) as the career unlock.
  ...
  **Build order:** **1.17** → then **3.1** / **3.1b** / **3.2**.
