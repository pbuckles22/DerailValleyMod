...
**Last full pass:** Epic 1 close remediation — 2026-07-23.
...
- [ ] **Cache player XZ per HUD tick** — `TryGetPlayerPosition` re-runs from Pos + Marked labels and both T2 snapshots each refresh (**1.12–1.14**). Cache in `BeginHudTick` like standing/loco.
- [ ] **Extract TelemetryReader subsystems** — ~1512 LOC after Epic 1 (**1.7–1.14** fluids, limit scan, heading, pos, park). Split when editing (limit scan first; nav/park second).
...
- [ ] **MonitorHudDriver Tier 2 emit boilerplate** — nine near-identical `Emit*DebugIfNeeded` blocks (incl. **1.14** `T2 mark`). Extract shared previous-snapshot + log helper before **4.6** or next T2 channel.
