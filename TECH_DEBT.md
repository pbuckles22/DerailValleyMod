...
- [ ] **Limit scan hitch (0.6.33+)** — **0.6.49:** cab never FoT / never cold GetClosest (`LimitCabDiscoveryPolicy`); Align `WarmForPlan` sync ≤500 ms / ≤8 on-route owns discovery. Cab enter Limit clean in log. **0.6.50:** change-driven `T2 …` telemetry (+ board traces) now opt-in via **Shift+F2** (`Tier2TelemetryLogGate`) — look-around wrote ~10 lines/s of `Main.Log` I/O because look-at/heading follow the camera; awaiting look-around re-smoke. No Align ⇒ sticky/seed / `— Limit`. Next: confirm look-around PASS; motion-gated discovery without Align; after PASS strip `BoardCachePump` dead stubs.

- [ ] **Rhythmic ~2.5 s hitch (GC — primary; Tier 2 FAIL @ 0.6.54)** — Same cadence in/out of town, cab, on foot; **gone with mod disabled**. Radar/Limit FoT were wrong primaries for *this* symptom (cab FoT + settings fixes still useful). **0.6.50–0.6.54 (uncommitted):** opt-in Tier 2 logs; once-per-town radar; OnGUI measure cache; AR measure/caption/layout alloc cuts; probe throttles; `T2 perf gc` probe. **Smoke still FAIL.** Next: dirty-check 10 Hz HUD (`MonitorHudDriver.Update` → train/always-on Format+Join); stop `FormatMotorsHudChip` flash forcing bar rebuild; alloc-free `TryGetJobsFromPlayerInventory`. Do not merge until PASS/waive.

- [ ] **Settings.xml never loaded (fixed 0.6.51, verify in log)** — `YardMasterSuite.ModSettings` collided with UMM's `ModSettings` XML type name, so load **and** save threw every session: UMM options silently reset to defaults (`ShowNearestLocos = true` even if unchecked) and the unguarded throw in `OnSaveGUI` is the likely reason the Mod Manager stopped responding to Ctrl+F10 after toggling the mod off. Fixed with `[XmlRoot]`/`[XmlType]` + try/catch around Save. No Tier 1 coverage possible (Unity-side type) — verify via absence of `Can't read … Settings.xml`.
