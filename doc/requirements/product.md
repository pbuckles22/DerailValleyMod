...
**1.10 / 1.11 / 1.16 notes:** Single `Limit` badge (never two limit numbers). Posted boards (digit × 10; facing our travel; dual `6/4` via branch) are the authored authority. **1.16** = Limit is a **look-ahead recommendation**: adopts the next slower board inside the soft-brake lead (not at tire pass), and may tighten from **sustained** track geometry (SignPlacer ladder; zones &lt; 15 m ignored). Soft brake chip `Brake N in M m` yellow→red beside Limit. Yellow within 5 km/h of Limit; red when over. **1.11** trend ↑/↓ on the same badge for a further different board. Hidden with the top bar (**4.3**).
