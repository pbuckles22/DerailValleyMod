...
**1.10 / 1.11 / 1.16 notes:** Single `Limit` badge (never two limit numbers). Posted boards (digit × 10; facing our travel; dual `6/4` via branch) are the authored authority. **1.16** = Limit is a **look-ahead recommendation**: adopts the next slower posted board inside the soft-brake lead (not at tire pass), and may tighten from **sustained** track geometry (SignPlacer ladder; zones &lt; 15 m ignored). Brake must inspect all route-ahead boards so a nearby mild drop cannot hide a farther severe one; its window is the worse of a comfortable slowdown and the room a heavy application actually needs, plus a **10% buffer**, computed from current speed, consist mass, grade, and locomotive profile. Soft brake chip `Brake N in M m` is yellow→red beside Limit and is driven by **our speed**, not by the Limit chip: `Limit N (Recommended)` and `Brake N in … s` appear together, and the countdown never disappears because the number was adopted. Once shown, a target holds until it is passed or we are slow enough for it — no frame-to-frame blinking from grade noise. Limit caution is asymmetric: yellow from **Limit−10 through Limit+5** (inclusive), red above **Limit+5**; Brake shares that same tolerance and never turns red for a board while Limit itself is still only yellow. **Route-ahead boards do not depend on their sign having rendered:** the game streams posted-sign props in only near the train, but the `RailTrack` curve graph is not streamed, so the look-ahead scan also derives a synthetic board from curve-radius geometry (SignPlacer ladder) for every walked segment — this is what lets a severe restriction (e.g. 70→30) warn kilometres out even before its physical sign exists in the scene. **1.11** trend ↑/↓ on the same badge for a further different board. Hidden with the top bar (**4.3**).
