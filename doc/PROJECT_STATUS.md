# Project status

Human-readable **snapshot**. Keep in sync with [AGENT_HANDOFF.md](../AGENT_HANDOFF.md) → *Current state* and checkbox truth in [PM_PLAN.md](../PM_PLAN.md).

**Last updated:** 2026-08-03

---

## Summary

**Yard Master Suite** — Fleet Operator utilities for *Derail Valley*  
Stack: Unity / C# `net48` / UMM / Harmony  

| | |
|--|--|
| **Journey** | Stage 3 — next **3.1** / **3.1b** / **3.2** |
| **MVP** | Epic 1 System Monitor HUD |
| **Version** | **0.5.105** *(Stress RAG on `main`)* |
| **Active branch** | `main` |

---

## Active work

| Branch | Role |
|--------|------|
| **`main`** | Integration — **0.5.105** (Stress RAG Tier 2 PASS) |

---

## Progress (official IDs)

### Epics

- [x] **Epic 0** — Foundation & Safe Boot
- [x] **Epic 1** — Diagnostic HUD *(**1.15** done; **1.16** CUT; **1.17** done @ **0.5.104**; Stress RAG @ **0.5.105**)*
- [x] **Epic 2** — Governor Mode *(complete 2026-07-28; **2.3** @ **0.5.25**)*
- [ ] **Epic 3** — Yard Master / Dispatcher *(**3.5** done @ **0.5.101**)*
- [x] **Epic 4** — HUD quality *(complete 2026-07-28 incl. **4.12** @ **0.5.17**)*
- [ ] **Epic 5** — Digital Catalog

### Next

1. **[ ] 3.1 / 3.1b / 3.2** — Manual consist / license re-rail / …
2. Parking: flight-sim HUD; AR loco ≤1 km / house hide; DM3; session reset; 1.17 #4 behind-seed ice
