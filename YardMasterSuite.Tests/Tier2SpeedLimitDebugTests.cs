...
public class Tier2SpeedLimitDebugTests
{
    ...
    [Fact]
    public void NextLogMessage_logs_when_next_chip_changes()
    {
        var prior = Live(limit: "Limit 70 | Next 80 (85m)");
        var next = Live(limit: "Limit 70 | Next 50 (200m)");
        var msg = Tier2SpeedLimitDebug.NextLogMessage(prior, next);
        Assert.NotNull(msg);
        Assert.Contains("Next 50", msg!);
    }
}
